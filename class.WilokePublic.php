...
//        add_action('admin_init', array($this, 'putDataToRedis'));
    ...
    public function loadmore_listing_layout(){
        ...
        if ( $aData['is_open_now'] === 'true' || $aData['is_highest_rated'] === 'true' || (!empty($aData['price_segment']) && ($aData['price_segment'] !== 'all')) ){
            ...
            if ( !empty($aData['target']) ) {
                switch ($aData['get_posts_from']){
                    case 'post_author':
                        $aArgs['author__in'] = $aData['target'];
                        break;
                    case 'custom':
                        $aArgs['post__in'] = $aData['target'];
                        break;
                    case 'listing_location':
                        if ( (!isset($aData['filter_by']) || !empty($aData['filter_by']) || $aData['listing_locations'] !== 'all') && empty($aData['term']) ){
                            $aArgs['tax_query'][] = array(
                        ...
                        break;
                    case 'listing_cat':
                        if ( !isset($aData['filter_by']) || !empty($aData['filter_by']) || $aData['listing_cats'] !== 'all' ){
                            $hasCat = true;
                            $aArgs['tax_query'][] = array(
            ...
            if ( isset($aData['filter_by']) && !empty($aData['filter_by']) ) {
                if ( ($aData['term'] !== 'all') ){
                    $aArgs['tax_query'][] = array(
        ...
        if ( !empty($aData['listing_locations']) ){
            ...
            if ( !isset($ignoreParseLocation) ){
                if ( is_array($aData['listing_locations']) ){
                    ...
                    $postIDsInLocation = "SELECT $termRelationShipsTbl.object_id FROM $termRelationShipsTbl WHERE $termRelationShipsTbl.term_taxonomy_id IN {$listingLocationID}";
                }else{
                    $listingLocationID =  absint($aData['listing_locations']);
                    $postIDsInLocation = "SELECT $termRelationShipsTbl.object_id FROM $termRelationShipsTbl WHERE $termRelationShipsTbl.term_taxonomy_id = {$listingLocationID}";
                }
    ...
    public static function listingQuery($atts, $mapPage='', $isGetInfo=true, $postID=null){
        ...
        $highliht_list = get_post_meta($post->ID, 'wiloke_listgo_toggle_highlight', true);
        if ( file_exists(get_template_directory() . '/admin/public/template/vc/listing-layout/'.$atts['layout'].'.php') ){
            if($highliht_list)
                ...
                //include get_template_directory() . '/admin/public/template/vc/listing-layout/'.$atts['layout'].'2.php';
                return false;
        }else{
            include get_template_directory() . '/admin/public/template/vc/listing-layout/rest-layouts.php';
        }
    }
    public static function listingQuery_borbis($atts, $mapPage='', $isGetInfo=true, $postID=null){
        global $wiloke;
        $aInfo = $terms = array();
        if ( empty($postID) ){
            global $post;
    ...
    public static function renderFavorite($post, $atts=null, $additionalClass=''){
        ...
        if ( !empty($atts) ){
            ...
            $text = $atts['favorite_description'];
        }
        ...
        <form action="<?php echo esc_url($mapPage); ?>" method="GET" id="listgo-searchform" class="form form--listing <?php echo esc_attr($currentlySearchOn); ?>">
            ...
            <div class="form-item item--search">
                ...
                <span class="input-text input-icon-inside">
                    <input id="<?php echo esc_attr($inputName); ?>" type="text" name="<?php echo esc_attr($inputName); ?>" value="<?php echo esc_attr($s); ?>">
                    <?php
    ...
    public static function renderLocationField($locationFieldTitle, $aLocations){
        ...
        <div class="form-item item--localtion">
            ...
            <span class="input-text input-icon-inside">
                <input type="text" id="s_listing_location" class="auto-location-by-google" name="s_listing_location" placeholder="<?php echo esc_html($locationFieldTitle); ?>" value="<?php echo esc_attr($termName); ?>">
    ...
    public static function headerPage(){
        ...
        <div class="header-page bg-scroll lazy" data-src="<?php echo esc_url($headerURL); ?>">
            ...
            <div class="header-page__inner">
                <h2 class="header-page__title"><span><?php echo esc_html($postTitle); ?></span></h2>
                <?php if ( !empty($desc) ) : ?>
        ...
        if ( !isset($wiloke->aThemeOptions['listing_toggle_search_by_tag']) || ($wiloke->aThemeOptions['listing_toggle_search_by_tag'] == 'enable') ) :
            ...
            if ( !empty($aTags) && !is_wp_error($aTags) ) : ?>
                <div class="form-item item--tags">
                   <label class="label"><?php esc_html_e('Sortuj', 'listgo'); ?> <span class="toggle-tags"></span></label>
    ...
    public static function renderFindDirection($aPageSettings, $atts=null, $additionalClass=''){
        ...
        if ( !empty($atts) ){
            ...
            $text = empty($atts['find_direction_text']) ? esc_html__('Pokaż dojazd', 'listgo') : $atts['find_direction_text'];
        }
    ...
    public function submitReview(){
        ...
        if ( empty($userID) ){
            if ( empty($aData['email']) ){
                ...
                $aResult = WilokeSubmissionUser::createUserByEmail($aData['email']);
        ...
        <li class="comment" data-reviewid="<?php echo esc_attr($oResult->ID); ?>">
            <div class="comment__inner">
                ...
                <div class="comment__body">
                    ...
                    <span class="comment__edit-reply">
                        <a class="wiloke-listgo-thanks-for-reviewing comment-like <?php echo esc_attr($actived); ?>" data-id="<?php echo esc_attr($oResult->ID); ?>" href="#">
                            <i class="icon_like"></i><span class="comment-like__count"><?php echo esc_html($countThanks); ?></span>
                        </a>
                    </span>
                </div>
            </div>
            ...
            <div class="tb">
                <div class="tb__cell">
                    <div class="user__icon" data-modal="#modal-login">
                        <img src="<?php echo esc_url(get_template_directory_uri() . '/img/icon-user.png') ?>" alt="<?php esc_html_e('Login Register', 'listgo'); ?>">
                    </div>
                </div>
            </div>
        </div>
	    ...
	    if ( !empty($wiloke->aThemeOptions) && class_exists('ReduxFrameworkPlugin') ){
            ...
            switch ($aArgs['sidebar']){
                ...
                case 'right':
                    ...
                    break;
                default:
                    $aArgs['main_class'] = $aArgs['layout'] === 'post__standard' ? 'col-md-12' : 'col-md-12';
                    break;
            }
